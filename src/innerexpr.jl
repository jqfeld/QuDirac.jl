...
inner_rule(::P, b, k) where P <: AbstractInner = error("inner_rule(::$P, ::StateLabel, ::StateLabel) must be defined to evaluate inner products with type $P")
inner_rule(::UndefinedInner, b, k) = InnerExpr(InnerProduct(b, k))
inner_rule(::KroneckerDelta, b, k) = b == k ? 1 : 0

inner_rettype(::P) where {P<:AbstractInner} = first(Base.return_types(inner_rule, (P, StateLabel, StateLabel)))
...
struct InnerExpr <: Number
    ex::Expr
end

InnerExpr(iex::InnerExpr) = InnerExpr(iex.ex)
InnerExpr(n::N) where {N<:Number} = convert(InnerExpr, n)

Base.convert(::Type{InnerExpr}, iex::InnerExpr) = iex
Base.convert(::Type{InnerExpr}, n::N) where {N<:Number} = InnerExpr(Expr(:call, +, n))
...
Base.promote_rule(::Type{InnerExpr}, ::Type{N}) where {N<:Number} = InnerExpr
...
Base.:^(a::typeof(MathConstants.e), b::InnerExpr) = iexpr_exp(a, b)
...
Base.log(a::typeof(MathConstants.e), b::InnerExpr) = InnerExpr(:(log($(a),$(b))))
...
Base.adjoint(iex::InnerExpr) = conj(iex)

##########################
# Elementwise Operations #
##########################
# still necessary in modern julia?
# for op=(:*,:-,:+,:/,:^)
